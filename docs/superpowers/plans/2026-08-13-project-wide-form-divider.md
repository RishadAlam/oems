...
**Architecture:** Keep the two established form families and change their shared CSS contract from section bottom borders to later-sibling top borders. Test the compiled stylesheet that browsers consume, then publish it under a fresh cache key in all layouts and the service worker.
...
- [ ] **Step 2: Run the focused test to verify it fails for the current browser CSS**
...
- Produces: the cache-key contract `20260813-form-divider-v1`

- [ ] **Step 1: Change cache delivery expectations first**

Replace every expected CSS/cache version `20260813-admin-filter-alignment-v1` in the three test files with `20260813-form-divider-v1`.

- [ ] **Step 2: Run delivery tests to verify they fail against production files**
...
- [ ] **Step 3: Commit the failing delivery contract**
...
- [ ] **Step 1: Replace bottom-border section rules with later-sibling top-border rules**
...
Remove the obsolete `:last-of-type` rules. Leave `profile-form-actions` and `organizer-form__actions` with their existing top borders.

- [ ] **Step 2: Publish the new cache key everywhere**

Replace `20260813-admin-filter-alignment-v1` with `20260813-form-divider-v1` in all four layouts and `public/service-worker.js`.
...
- [ ] **Step 1: Inspect representative rendered forms**
...
- [ ] **Step 3: Mark every completed plan checkbox and record the test totals**
