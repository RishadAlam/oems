        ...
        <?php else: ?>
            <div class="grid gap-3">
                <?php foreach ($recentTickets as $item): ?>
                    <?php $ticketStatus = (string) ($item['ticket_status'] ?? ''); ?>
                    <a class="flex flex-wrap items-center justify-between gap-4 rounded-[18px] border border-[var(--line)] p-4 transition hover:border-[var(--accent)]" href="/participant/tickets/<?= e((int) ($item['id'] ?? 0)) ?>"><span class="min-w-0 flex-1"><strong class="block break-words"><?= e($item['event_title'] ?? '') ?></strong><small class="break-all text-[var(--ink-muted)]"><?= e($item['ticket_number'] ?? '') ?></small></span><span class="status-badge status-badge--<?= e(status_modifier($ticketStatus, 'ticket')) ?> shrink-0"><?= e(oems_status_label($ticketStatus)) ?></span><i class="ph ph-arrow-right shrink-0" aria-hidden="true"></i></a>
                <?php endforeach; ?>
            </div>
