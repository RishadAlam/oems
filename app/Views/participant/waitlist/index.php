    ...
    <div class="mt-8 grid gap-5 lg:grid-cols-2">
        ...
        ?>
            <article class="dashboard-panel" aria-labelledby="waitlist-event-<?= e($entry['id']) ?>">
                ...
                <form class="form-stack mt-5" action="/participant/waitlist/<?= e($entry['id']) ?>/leave" method="post" novalidate>
                    <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                    <div class="field-group">
                        <label for="<?= e($reasonId) ?>">Reason for leaving</label>
                        <textarea id="<?= e($reasonId) ?>" name="reason" rows="2" maxlength="500" required aria-describedby="waitlist-reason-help-<?= e($entry['id']) ?><?= $reasonError !== null ? ' waitlist-reason-error-' . e($entry['id']) : '' ?>"<?= $reasonError !== null ? ' aria-invalid="true"' : '' ?>><?= (int) ($old['registration_id'] ?? 0) === (int) $entry['id'] ? e($old['reason'] ?? '') : '' ?></textarea>
                    ...
                    <button class="button button--danger w-full sm:w-auto" type="submit"><i class="ph ph-sign-out" aria-hidden="true"></i><span>Leave waitlist</span></button>
                </form>
            </article>
        <?php endforeach; ?>
    </div>
