...
<?php else: ?>
    <div class="mt-8 grid gap-4">
        <?php foreach ($tickets as $ticket): ?>
            <?php $ticketStatus = (string) ($ticket['ticket_status'] ?? ''); ?>
            <article class="dashboard-panel flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex flex-wrap items-center gap-3"><span class="status-chip status-chip--<?= e(status_modifier($ticketStatus, 'ticket')) ?>"><?= e(oems_status_label($ticketStatus, $statusLabels)) ?></span><a class="button button--primary button--compact" href="/participant/tickets/<?= e($ticket['id']) ?>">Open ticket</a></div>
            </article>
        <?php endforeach; ?>
    </div>
<?php endif; ?>
