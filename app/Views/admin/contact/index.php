...
<section class="dashboard-panel mt-8">
    ...
    <?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><caption class="sr-only">Contact messages</caption><thead><tr><th>Sender</th><th>Subject</th><th>Status</th><th>Received</th><th>Action</th></tr></thead><tbody>
        <?php foreach ($messages as $message): ?><?php $messageStatus = (string) ($message['status'] ?? ''); ?><tr><td data-label="Sender"><div class="organizer-table__primary min-w-0 [overflow-wrap:anywhere]"><strong><?= e($message['name']) ?></strong><span class="mt-1 block break-words text-sm text-[var(--ink-muted)]"><?= e($message['email']) ?></span></div></td><td class="[overflow-wrap:anywhere]" data-label="Subject"><?= e($message['subject']) ?></td><td data-label="Status"><span class="status-badge status-badge--<?= e(status_modifier($messageStatus, 'contact')) ?>"><?= e(oems_status_label($messageStatus)) ?></span></td><td data-label="Received"><?= e($message['created_at']) ?></td><td class="organizer-table__action" data-label="Action"><a class="button button--compact button--quiet" href="/admin/contact/<?= e($message['id']) ?>">Review</a></td></tr><?php endforeach; ?>
        </tbody></table></div>
    <?php endif; ?>
</section>
