    ...
    <aside class="dashboard-panel organizer-actions-panel" aria-labelledby="payment-actions-heading">
        ...
        <?php if ($status === 'pending'): ?>
            ...
            <?php else: ?><div class="organizer-action-stack">
                <form action="/admin/payments/<?= e($payment['id']) ?>/verify" method="post" data-form-kind="entry"><input type="hidden" name="_token" value="<?= e($csrfToken) ?>"><?php foreach ($preserved as $name => $value): ?><input type="hidden" name="<?= e($name) ?>" value="<?= e($value) ?>"><?php endforeach; ?><div class="field-group"><label for="verify-note">Verification note <span class="text-[var(--ink-muted)]">(optional)</span></label><textarea id="verify-note" name="note" rows="4" maxlength="500" data-form-label="Verification note" aria-describedby="verify-note-help<?= field_error($errors, 'note') === null ? '' : ' verify-note-error' ?>"><?= old_value($old, 'note') ?></textarea><p id="verify-note-help" class="field-help">Add only settlement context. Do not include account credentials.</p><?php if ($error = field_error($errors, 'note')): ?><p id="verify-note-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div><button class="button button--primary w-full" type="submit" data-submit-label="Reviewing verification…"><i class="ph ph-check-circle" aria-hidden="true"></i><span data-submit-text>Review verification</span></button></form>
                <form action="/admin/payments/<?= e($payment['id']) ?>/reject" method="post" data-form-kind="entry"><input type="hidden" name="_token" value="<?= e($csrfToken) ?>"><?php foreach ($preserved as $name => $value): ?><input type="hidden" name="<?= e($name) ?>" value="<?= e($value) ?>"><?php endforeach; ?><div class="field-group"><label for="reject-note">Rejection note <span class="text-[var(--ink-muted)]">(optional)</span></label><textarea id="reject-note" name="note" rows="4" maxlength="500" data-form-label="Rejection note" aria-describedby="reject-note-help<?= field_error($errors, 'note') === null ? '' : ' reject-note-error' ?>"><?= old_value($old, 'note') ?></textarea><p id="reject-note-help" class="field-help">Explain the mismatch without copying private account details.</p><?php if ($error = field_error($errors, 'note')): ?><p id="reject-note-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div><button class="button button--danger w-full" type="submit" data-submit-label="Reviewing rejection…"><i class="ph ph-x-circle" aria-hidden="true"></i><span data-submit-text>Review rejection</span></button></form>
            </div><?php endif; ?>
