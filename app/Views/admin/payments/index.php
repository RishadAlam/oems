...
<section class="dashboard-panel organizer-list-panel mt-6" aria-labelledby="payment-list-heading">
    ...
    <?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><caption class="sr-only">Administrator payment review records</caption><thead><tr><th scope="col">Participant</th><th scope="col">Event</th><th scope="col">Amount</th><th scope="col">Method</th><th scope="col">Reference</th><th scope="col">Status</th><th scope="col">Action</th></tr></thead><tbody>
        <?php foreach ($payments as $payment): ?><?php $paymentStatus = (string) ($payment['payment_status'] ?? ''); ?><tr><td data-label="Participant"><strong><?= e($payment['participant_name'] ?? 'Participant') ?></strong><small class="organizer-table__value"><?= e($payment['participant_email'] ?? '') ?></small></td><td data-label="Event"><strong><?= e($payment['event_title'] ?? 'Event') ?></strong><small><?= e($payment['organizer_name'] ?? 'Organizer') ?></small></td><td data-label="Amount"><strong><?= e($payment['currency'] ?? 'BDT') ?> <?= e($payment['amount'] ?? '0.00') ?></strong></td><td data-label="Method"><strong><?= e($payment['payment_method_name'] ?? 'Manual payment') ?></strong><small><?= e(ucwords(str_replace('_', ' ', (string) ($payment['payment_channel'] ?? 'Not supplied')))) ?></small></td><td class="organizer-table__value" data-label="Reference"><strong><?= e($payment['transaction_reference'] ?? 'Not supplied') ?></strong><small><time datetime="<?= e(str_replace(' ', 'T', (string) ($payment['created_at'] ?? ''))) ?>"><?= e($payment['created_at'] ?? '') ?></time></small></td><td data-label="Status"><span class="status-chip status-chip--<?= e(status_modifier($paymentStatus, 'payment')) ?>"><?= e(oems_status_label($paymentStatus, $statusLabels)) ?></span></td><td class="organizer-table__action" data-label="Action"><a class="text-link" href="/admin/payments/<?= e($payment['id']) ?>?<?= e($query(['page' => $page])) ?>">Review <i class="ph ph-arrow-right" aria-hidden="true"></i></a></td></tr><?php endforeach; ?>
        </tbody></table></div>
