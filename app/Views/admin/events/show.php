...
<div class="dashboard-page-heading organizer-page-heading">
    <div>
        ...
        <h1><?= e($event['title']) ?></h1>
        <p>Current status: <span class="status-chip status-chip--<?= e(status_modifier($status, 'event')) ?>"><?= e(oems_status_label($status, $statusLabels)) ?></span></p>
    </div>
...
<div class="admin-moderation-layout mt-8">
    <article class="dashboard-panel admin-evidence-panel" aria-labelledby="event-evidence-heading">
        ...
        <div class="admin-evidence-summary"><span class="status-chip status-chip--<?= e(status_modifier($status, 'event')) ?>"><?= e(oems_status_label($status, $statusLabels)) ?></span><span><?= e($event['category_name'] ?? 'Uncategorized') ?></span><span><?= e($event['organization_name'] ?? 'Unknown organizer') ?></span></div>
    ...
    <aside class="dashboard-panel organizer-actions-panel" aria-labelledby="moderation-actions-heading">
        ...
        <p class="admin-current-state">Current status: <span class="status-chip status-chip--<?= e(status_modifier($status, 'event')) ?>"><?= e(oems_status_label($status, $statusLabels)) ?></span></p>
