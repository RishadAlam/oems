...
<section class="dashboard-panel mt-8" aria-labelledby="campaigns-heading">
    ...
    <?php else: ?>
        <div class="table-shell mt-6"><table class="operations-table"><thead><tr><th>Campaign</th><th>Status</th><th>Recipients</th><th>Created</th><th>Action</th></tr></thead><tbody>
        <?php foreach ($campaigns as $campaign): ?><tr><td data-label="Campaign"><strong><?= e($campaign['subject']) ?></strong><p class="mt-2 max-w-xl whitespace-pre-wrap text-sm text-[var(--ink-muted)]"><?= e($campaign['message']) ?></p></td><td data-label="Status"><span class="status-badge status-badge--<?= e($campaign['status']) ?>"><?= e(ucfirst($campaign['status'])) ?></span></td><td data-label="Recipients"><?= e((int) $campaign['queued_count']) ?> of <?= e((int) $campaign['recipient_count']) ?></td><td data-label="Created"><?= e($campaign['created_at']) ?></td><td data-label="Action"><?php if ($campaign['status'] === 'draft'): ?><form method="post" action="/admin/newsletter/<?= e($campaign['id']) ?>/queue" data-form-kind="action" data-confirm="Queue this campaign for all currently confirmed subscribers?"><input type="hidden" name="_token" value="<?= e($csrfToken) ?>"><button class="button button--compact button--primary" type="submit" data-submit-label="Queueing campaign…"><span data-submit-text>Queue campaign</span></button></form><?php else: ?><span class="text-sm text-[var(--ink-muted)]">No action needed</span><?php endif; ?></td></tr><?php endforeach; ?>
        </tbody></table></div>
    <?php endif; ?>
</section>
