...
<section class="dashboard-panel analytics-filter mt-8" aria-labelledby="organizer-analytics-filters">
    <div class="analytics-filter__heading"><div class="dashboard-panel__heading-main"><span class="dashboard-panel__icon"><i class="ph ph-funnel" aria-hidden="true"></i></span><div><h2 id="organizer-analytics-filters">Analysis controls</h2><p>Choose up to 366 inclusive calendar days.</p></div></div><p class="analytics-filter__range" data-analytics-applied-range><i class="ph ph-calendar-blank" aria-hidden="true"></i><span>Applied range <strong><?= e($range['start'] ?? '') ?> to <?= e($range['end'] ?? '') ?></strong></span></p></div>
    <form class="analytics-filter__form analytics-filter__form--organizer" method="get" action="/organizer/analytics" data-form-kind="filter">
        ...
        <div class="analytics-filter__actions"><button class="button button--primary" type="submit">Apply</button><a class="button button--quiet" href="/organizer/analytics">Reset</a></div>
    </form>
</section>

<section class="analytics-kpi-grid mt-6" aria-label="Organizer analytics summary">
    <article class="analytics-kpi"><div class="analytics-kpi__top"><span class="analytics-kpi__icon"><i class="ph ph-calendar-check" aria-hidden="true"></i></span><span class="analytics-kpi__label">Events</span></div><strong data-analytics-kpi-value aria-label="Events: <?= e((int) ($lifecycle['total'] ?? 0)) ?>"><?= e((int) ($lifecycle['total'] ?? 0)) ?></strong><p><?= e((int) ($lifecycle['published'] ?? 0)) ?> published, <?= e((int) ($lifecycle['completed'] ?? 0)) ?> completed</p></article>
    <article class="analytics-kpi"><div class="analytics-kpi__top"><span class="analytics-kpi__icon"><i class="ph ph-ticket" aria-hidden="true"></i></span><span class="analytics-kpi__label">Registrations</span></div><strong data-analytics-kpi-value aria-label="Confirmed registrations: <?= e((int) ($registrations['confirmed'] ?? 0)) ?>"><?= e((int) ($registrations['confirmed'] ?? 0)) ?></strong><p><?= e((int) ($registrations['pending'] ?? 0)) ?> pending, <?= e((int) ($registrations['cancelled'] ?? 0)) ?> cancelled</p></article>
    <article class="analytics-kpi"><div class="analytics-kpi__top"><span class="analytics-kpi__icon"><i class="ph ph-gauge" aria-hidden="true"></i></span><span class="analytics-kpi__label">Capacity utilization</span></div><strong data-analytics-kpi-value aria-label="Capacity utilization: <?= e($summary['capacity_utilization_rate'] ?? '0.0') ?> percent"><?= e($summary['capacity_utilization_rate'] ?? '0.0') ?>%</strong><p><?= e((int) ($summary['capacity_total'] ?? 0)) ?> total places</p></article>
    <article class="analytics-kpi"><div class="analytics-kpi__top"><span class="analytics-kpi__icon"><i class="ph ph-check-circle" aria-hidden="true"></i></span><span class="analytics-kpi__label">Attendance rate</span></div><strong data-analytics-kpi-value aria-label="Attendance rate: <?= e($summary['attendance_rate'] ?? '0.0') ?> percent"><?= e($summary['attendance_rate'] ?? '0.0') ?>%</strong><p><?= e((int) ($summary['attendance_count'] ?? 0)) ?> checked in</p></article>
</section>

<?php require base_path('app/Views/components/analytics-charts.php'); ?>

<div class="mt-6 grid gap-6 xl:grid-cols-2">
    <section class="dashboard-panel" aria-labelledby="organizer-lifecycle-breakdown"><div class="dashboard-panel__heading"><span class="dashboard-panel__icon"><i class="ph ph-flow-arrow" aria-hidden="true"></i></span><div><h2 id="organizer-lifecycle-breakdown">Lifecycle breakdown</h2><p>Every event state in the selected range.</p></div></div><dl class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4"><?php foreach ($lifecycleLabels as $status => $label): ?><div class="rounded-xl border border-[var(--line)] p-3" data-lifecycle-status="<?= e($status) ?>"><dt><span class="status-chip status-chip--<?= e(status_modifier($status, 'event')) ?>"><?= e($label) ?></span></dt><dd class="mt-1 text-xl font-bold"><?= e((int) ($lifecycle[$status] ?? 0)) ?></dd></div><?php endforeach; ?></dl></section>
    <section class="dashboard-panel" aria-labelledby="organizer-registration-breakdown"><div class="dashboard-panel__heading"><span class="dashboard-panel__icon"><i class="ph ph-users-three" aria-hidden="true"></i></span><div><h2 id="organizer-registration-breakdown">Registration breakdown</h2><p>Every registration state recorded in the range.</p></div></div><dl class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3"><?php foreach ($registrationLabels as $status => $label): ?><div class="rounded-xl border border-[var(--line)] p-3" data-registration-status="<?= e($status) ?>"><dt><span class="status-chip status-chip--<?= e(status_modifier($status, 'registration')) ?>"><?= e($label) ?></span></dt><dd class="mt-1 text-xl font-bold"><?= e((int) ($registrations[$status] ?? 0)) ?></dd></div><?php endforeach; ?></dl></section>
</div>
...
<section class="dashboard-panel organizer-list-panel mt-6" aria-labelledby="event-analytics-breakdown">
    ...
    <?php if ($rows === []): ?><div class="empty-state"><span class="empty-state__icon"><i class="ph ph-chart-line" aria-hidden="true"></i></span><strong>No event activity</strong><p>Adjust the dates or remove the event filter.</p></div><?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><caption class="sr-only">Organizer event analytics breakdown</caption><thead><tr><th scope="col">Event</th><th scope="col">Lifecycle</th><th scope="col">Registrations</th><th scope="col">Attendance</th><th scope="col">Payments</th><th scope="col">Engagement</th></tr></thead><tbody>
        <?php foreach ($rows as $row): $counts = (array) ($row['registration_counts'] ?? []); $eventStatus = (string) ($row['event_status'] ?? ''); ?>
            <tr>
                <td data-label="Event"><strong><?= e($row['event_title'] ?? 'Event') ?></strong><small class="organizer-table__value">#<?= e((int) ($row['event_id'] ?? 0)) ?>, <?= e($row['start_date'] ?? '') ?><?= !empty($row['archived']) ? ', archived' : '' ?></small></td>
                <td data-label="Lifecycle"><span class="status-chip status-chip--<?= e(status_modifier($eventStatus, 'event')) ?>"><?= e(oems_status_label($eventStatus)) ?></span><small>Capacity <?= e((int) ($row['capacity'] ?? 0)) ?></small></td>
