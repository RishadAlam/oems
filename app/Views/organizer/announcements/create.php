...
<div class="admin-moderation-layout mt-8">
    <section class="dashboard-panel" aria-labelledby="announcement-compose-heading">
        <?php if (is_array($confirmation ?? null)): ?>
            ...
            <form class="mt-6" action="/organizer/events/<?= e($event['id']) ?>/announcements" method="post">
                <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                <input type="hidden" name="confirm_send" value="1">
                <input type="hidden" name="request_key" value="<?= e($confirmation['request_key'] ?? '') ?>">
                <button class="button button--primary w-full" type="submit"><i class="ph ph-paper-plane-tilt" aria-hidden="true"></i><span>Confirm send</span></button>
            </form>
        ...
        <?php else: ?>
            ...
            <?php if ($canSend): ?>
                <form class="mt-6 grid gap-5" action="/organizer/events/<?= e($event['id']) ?>/announcements" method="post">
                    <input type="hidden" name="_token" value="<?= e($csrfToken) ?>">
                    <div class="field-group">
                        <label for="announcement-subject">Subject</label>
                        <input id="announcement-subject" name="subject" type="text" maxlength="180" value="<?= old_value($old, 'subject') ?>" autocomplete="off" required<?= field_error($errors, 'subject') === null ? '' : ' aria-invalid="true" aria-describedby="announcement-subject-error"' ?>>
                    ...
                    <div class="field-group">
                        ...
                        <textarea id="announcement-message" name="message" rows="9" maxlength="1000" aria-describedby="announcement-message-help<?= field_error($errors, 'message') === null ? '' : ' announcement-message-error' ?>" required><?= old_value($old, 'message') ?></textarea>
                    ...
                    <button class="button button--primary" type="submit"><i class="ph ph-magnifying-glass" aria-hidden="true"></i><span>Review announcement</span></button>
                </form>
            <?php else: ?>
