...
<form class="dashboard-panel organizer-form mt-8" action="<?= e($action) ?>" method="post" enctype="multipart/form-data" data-form-kind="entry">
    ...
    <?php
    $fieldTargets = ['event' => 'event-basics-heading'];
    $fieldLabels = [
        'event' => 'Event', 'category_id' => 'Category', 'venue_id' => 'Venue', 'title' => 'Event title',
        'description' => 'Description', 'map_url' => 'Map URL', 'speaker' => 'Speaker', 'start_date' => 'Starts',
        'end_date' => 'Ends', 'registration_deadline' => 'Response deadline', 'capacity' => 'Capacity',
        'ticket_price' => 'Ticket price', 'location_visibility' => 'Location visibility', 'arrival_notes' => 'Arrival notes',
        'waitlist_enabled' => 'Waitlist', 'tags' => 'Tags', 'banner' => 'Banner image', 'gallery' => 'Gallery images',
    ];
    $formErrorSummaryId = 'event-editor-error-summary';
    require base_path('app/Views/components/form-errors.php');
    ?>

    <p class="form-required-note"><i class="ph ph-asterisk" aria-hidden="true"></i><span>Event details, schedule, capacity, price, and location visibility are required.</span></p>
    ...
    <section class="organizer-form__section" aria-labelledby="event-basics-heading">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div class="field-group sm:col-span-2"><label for="title">Event title</label><input id="title" name="title" type="text" minlength="5" maxlength="180" value="<?= $eventValue('title') ?>" data-form-label="Event title" required<?= $invalid('title') ?>><?php if ($error = field_error($errors, 'title')): ?><p id="title-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="category_id">Category</label><select id="category_id" name="category_id" data-form-label="Category" required<?= $invalid('category_id') ?>><option value="">Select category</option><?php foreach ($categories as $category): ?><option value="<?= e($category['id']) ?>" <?= $selected('category_id') === (string) $category['id'] ? 'selected' : '' ?>><?= e($category['name']) ?></option><?php endforeach; ?></select><?php if ($error = field_error($errors, 'category_id')): ?><p id="category-id-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="venue_id">Venue <span class="field-label-note">Optional</span></label><select id="venue_id" name="venue_id" data-form-label="Venue"<?= $invalid('venue_id') ?>><option value="">No venue selected</option><?php foreach ($venues as $venueOption): ?><option value="<?= e($venueOption['id']) ?>" <?= $selected('venue_id') === (string) $venueOption['id'] ? 'selected' : '' ?>><?= e($venueOption['name']) ?></option><?php endforeach; ?></select><?php if ($error = field_error($errors, 'venue_id')): ?><p id="venue-id-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
        </div>
        <div class="field-group"><label for="description">Description</label><textarea id="description" name="description" rows="8" minlength="30" maxlength="20000" data-form-label="Description" required<?= $described('description') ?>><?= $eventValue('description') ?></textarea><p id="description-help" class="field-help">Use at least 30 characters.</p><?php if ($error = field_error($errors, 'description')): ?><p id="description-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
        <div class="grid gap-5 sm:grid-cols-2">
            <div class="field-group"><label for="speaker">Speaker <span class="field-label-note">Optional</span></label><input id="speaker" name="speaker" type="text" maxlength="190" value="<?= $eventValue('speaker') ?>" data-form-label="Speaker"<?= $invalid('speaker') ?>><?php if ($error = field_error($errors, 'speaker')): ?><p id="speaker-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="tags">Tags <span class="field-label-note">Optional</span></label><input id="tags" name="tags" type="text" maxlength="500" value="<?= $eventValue('tags') ?>" placeholder="design, community" data-form-label="Tags"<?= $described('tags') ?>><p id="tags-help" class="field-help">Separate up to 12 tags with commas.</p><?php if ($error = field_error($errors, 'tags')): ?><p id="tags-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
        </div>
    </section>
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <fieldset class="field-group">
                ...
                <div class="location-visibility-options">
                    <label>
                        <input id="location_visibility_public" name="location_visibility" type="radio" value="public" data-form-label="Location visibility" required<?= $locationVisibility === 'public' ? ' checked' : '' ?><?= $described('location_visibility') ?>>
                    ...
                    <label>
                        <input id="location_visibility_registered" name="location_visibility" type="radio" value="registered" data-form-label="Location visibility"<?= $locationVisibility === 'registered' ? ' checked' : '' ?><?= $described('location_visibility') ?>>
            ...
            <div class="field-group">
                ...
                <textarea id="arrival_notes" name="arrival_notes" rows="4" maxlength="500" data-form-label="Arrival notes"<?= $described('arrival_notes') ?>><?= $eventValue('arrival_notes') ?></textarea>
    ...
    <section class="organizer-form__section" aria-labelledby="event-schedule-heading">
        ...
        <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            <div class="field-group"><label for="start_date">Starts</label><input id="start_date" name="start_date" type="datetime-local" value="<?= $eventValue('start_date') ?>" data-form-label="Starts" required<?= $invalid('start_date') ?>><?php if ($error = field_error($errors, 'start_date')): ?><p id="start-date-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="end_date">Ends</label><input id="end_date" name="end_date" type="datetime-local" value="<?= $eventValue('end_date') ?>" data-form-label="Ends" data-after-field="start_date" required<?= $invalid('end_date') ?>><?php if ($error = field_error($errors, 'end_date')): ?><p id="end-date-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="registration_deadline">Response deadline</label><input id="registration_deadline" name="registration_deadline" type="datetime-local" value="<?= $eventValue('registration_deadline') ?>" data-form-label="Response deadline" data-before-or-equal-field="start_date" required<?= $invalid('registration_deadline') ?>><?php if ($error = field_error($errors, 'registration_deadline')): ?><p id="registration-deadline-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="capacity">Capacity</label><input id="capacity" name="capacity" type="number" min="1" max="100000" step="1" value="<?= $eventValue('capacity') ?>" data-form-label="Capacity" required<?= $invalid('capacity') ?>><?php if ($error = field_error($errors, 'capacity')): ?><p id="capacity-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="ticket_price">Ticket price</label><input id="ticket_price" name="ticket_price" type="number" min="0" max="9999999.99" step="0.01" value="<?= $eventValue('ticket_price', '0') ?>" data-form-label="Ticket price" required<?= $invalid('ticket_price') ?>><?php if ($error = field_error($errors, 'ticket_price')): ?><p id="ticket-price-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group sm:col-span-2 xl:col-span-1">
                <input type="hidden" name="waitlist_enabled" value="0">
                <label class="flex min-h-11 items-start gap-3" for="waitlist_enabled"><input id="waitlist_enabled" name="waitlist_enabled" type="checkbox" value="1" data-form-label="Waitlist"<?= $waitlistChecked ? ' checked' : '' ?>><span><strong>Enable a waitlist when seats sell out</strong><small class="mt-1 block text-[var(--ink-muted)]">Participants join without payment and are promoted oldest-first when a seat opens.</small></span></label>
            </div>
            <div class="field-group"><label for="map_url">Map URL <span class="field-label-note">Optional</span></label><input id="map_url" name="map_url" type="url" maxlength="500" placeholder="https://www.google.com/maps/..." value="<?= $eventValue('map_url') ?>" data-form-label="Map URL"<?= $invalid('map_url') ?>><p class="field-help">Use an HTTPS link from an approved map provider.</p><?php if ($error = field_error($errors, 'map_url')): ?><p id="map-url-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
        </div>
    </section>
    ...
    <section class="organizer-form__section" aria-labelledby="event-media-heading">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            <div class="field-group"><label for="banner">Banner image <span class="field-label-note">Optional</span></label><input id="banner" name="banner" type="file" accept="image/jpeg,image/png,image/webp" data-form-label="Banner image" data-max-bytes="5242880"<?= $invalid('banner') ?>><?php if ($error = field_error($errors, 'banner')): ?><p id="banner-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
            <div class="field-group"><label for="gallery">Gallery images <span class="field-label-note">Optional</span></label><input id="gallery" name="gallery[]" type="file" accept="image/jpeg,image/png,image/webp" multiple data-form-label="Gallery images" data-max-bytes="5242880" data-max-files="6"<?= $described('gallery') ?>><p id="gallery-help" class="field-help">Choose up to six images.<?php if ($isEdit && $gallery !== []): ?> New gallery images replace the current gallery.<?php endif; ?></p><?php if ($error = field_error($errors, 'gallery')): ?><p id="gallery-error" class="field-error" role="alert"><?= e($error) ?></p><?php endif; ?></div>
        </div>
