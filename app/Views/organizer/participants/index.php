...
<section class="dashboard-panel mt-6" aria-labelledby="participant-table-heading">
    ...
    <?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><thead><tr><th>Participant</th><th>Registration</th><th>Payment</th><th>Ticket</th><th>Attendance</th></tr></thead><tbody><?php foreach ($participants as $participant): ?><?php
            $registrationStatus = (string) ($participant['registration_status'] ?? '');
            $paymentStatus = (string) ($participant['payment_status'] ?? 'none');
            $ticketStatus = (string) ($participant['ticket_status'] ?? 'none');
            $attendanceStatus = (string) ($participant['attendance_status'] ?? 'not_checked_in');
        ?><tr><td data-label="Participant" id="participant-name-<?= e($participant['id']) ?>"><strong><?= e($participant['participant_name'] ?? '') ?></strong><small><?= e($participant['participant_email'] ?? '') ?></small></td><td data-label="Registration"><strong class="min-w-0 break-all"><?= e($participant['registration_number'] ?? '') ?></strong><small><span class="status-chip status-chip--<?= e(status_modifier($registrationStatus, 'registration')) ?>"><?= e($statusLabels[$registrationStatus] ?? ucfirst($registrationStatus)) ?></span></small></td><td data-label="Payment"><span class="status-chip status-chip--<?= e(status_modifier($paymentStatus, 'payment')) ?>"><?= e($statusLabels[$paymentStatus] ?? ucfirst($paymentStatus)) ?></span></td><td data-label="Ticket"><strong class="min-w-0 break-all"><?= e($participant['ticket_number'] ?? 'Not issued') ?></strong><small><span class="status-chip status-chip--<?= e(status_modifier($ticketStatus, 'ticket')) ?>"><?= e($statusLabels[$ticketStatus] ?? ucfirst($ticketStatus)) ?></span></small></td><td data-label="Attendance"><span class="status-chip status-chip--<?= e(status_modifier($attendanceStatus, 'attendance')) ?>"><?= e($statusLabels[$attendanceStatus] ?? ucfirst($attendanceStatus)) ?></span><?php if (!empty($participant['scanned_at'])): ?><small><?= e($participant['scanned_at']) ?></small><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
