...
<section class="dashboard-panel mt-8" aria-labelledby="participant-filters-heading">
    ...
    <form class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3" method="get" action="/organizer/events/<?= e($eventId) ?>/participants" data-form-kind="filter">
...
<section class="dashboard-panel mt-6" aria-labelledby="participant-table-heading">
    ...
    <?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><caption class="sr-only">Organizer event participants</caption><thead><tr><th scope="col">Participant</th><th scope="col">Registration</th><th scope="col">Payment</th><th scope="col">Ticket</th><th scope="col">Attendance</th></tr></thead><tbody><?php foreach ($participants as $participant): ?><?php
            $registrationStatus = (string) ($participant['registration_status'] ?? '');
            $paymentStatus = (string) ($participant['payment_status'] ?? '');
            $ticketStatus = (string) ($participant['ticket_status'] ?? '');
            $attendanceStatus = (string) ($participant['attendance_status'] ?? '');
        ?><tr><td data-label="Participant" id="participant-name-<?= e($participant['id']) ?>"><strong><?= e($participant['participant_name'] ?? '') ?></strong><small class="organizer-table__value"><?= e($participant['participant_email'] ?? '') ?></small></td><td class="organizer-table__value" data-label="Registration"><strong class="min-w-0 break-all"><?= e($participant['registration_number'] ?? '') ?></strong><small><span class="status-chip status-chip--<?= e(status_modifier($registrationStatus, 'registration')) ?>"><?= e(oems_status_label($registrationStatus, $statusLabels)) ?></span></small></td><td data-label="Payment"><span class="status-chip status-chip--<?= e(status_modifier($paymentStatus, 'payment')) ?>"><?= e(oems_status_label($paymentStatus, $statusLabels)) ?></span></td><td class="organizer-table__value" data-label="Ticket"><strong class="min-w-0 break-all"><?= e($participant['ticket_number'] ?? 'Not issued') ?></strong><small><span class="status-chip status-chip--<?= e(status_modifier($ticketStatus, 'ticket')) ?>"><?= e(oems_status_label($ticketStatus, $statusLabels)) ?></span></small></td><td data-label="Attendance"><span class="status-chip status-chip--<?= e(status_modifier($attendanceStatus, 'attendance')) ?>"><?= e(oems_status_label($attendanceStatus, $statusLabels)) ?></span><?php if (!empty($participant['scanned_at'])): ?><small><?= e($participant['scanned_at']) ?></small><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
