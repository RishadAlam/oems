...
<section class="dashboard-panel mt-6" aria-labelledby="participant-table-heading">
    ...
    <?php else: ?>
        <div class="organizer-table-wrap mt-6"><table class="operations-table organizer-table"><thead><tr><th>Participant</th><th>Registration</th><th>Payment</th><th>Ticket</th><th>Attendance</th></tr></thead><tbody><?php foreach ($participants as $participant): ?><tr><td data-label="Participant" id="participant-name-<?= e($participant['id']) ?>"><strong><?= e($participant['participant_name'] ?? '') ?></strong><small><?= e($participant['participant_email'] ?? '') ?></small></td><td data-label="Registration"><strong class="min-w-0 break-all"><?= e($participant['registration_number'] ?? '') ?></strong><small><?= e($statusLabels[$participant['registration_status'] ?? ''] ?? ucfirst((string) ($participant['registration_status'] ?? ''))) ?></small></td><td data-label="Payment"><?= e($statusLabels[$participant['payment_status'] ?? 'none'] ?? ucfirst((string) ($participant['payment_status'] ?? 'none'))) ?></td><td data-label="Ticket"><strong class="min-w-0 break-all"><?= e($participant['ticket_number'] ?? 'Not issued') ?></strong><small><?= e($statusLabels[$participant['ticket_status'] ?? 'none'] ?? ucfirst((string) ($participant['ticket_status'] ?? 'none'))) ?></small></td><td data-label="Attendance"><strong><?= e($statusLabels[$participant['attendance_status'] ?? 'not_checked_in'] ?? ucfirst((string) ($participant['attendance_status'] ?? 'not_checked_in'))) ?></strong><?php if (!empty($participant['scanned_at'])): ?><small><?= e($participant['scanned_at']) ?></small><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
