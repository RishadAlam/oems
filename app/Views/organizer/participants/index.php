...
<section class="dashboard-panel mt-6" aria-labelledby="participant-table-heading">
    ...
    <?php else: ?>
        ...
        ?><tr><td data-label="Participant" id="participant-name-<?= e($participant['id']) ?>"><strong><?= e($participant['participant_name'] ?? '') ?></strong><small><?= e($participant['participant_email'] ?? '') ?></small></td><td data-label="Registration"><strong class="min-w-0 break-all"><?= e($participant['registration_number'] ?? '') ?></strong><small><span class="status-chip status-chip--<?= e($registrationStatus) ?>"><?= e($statusLabels[$registrationStatus] ?? ucfirst($registrationStatus)) ?></span></small></td><td data-label="Payment"><span class="status-chip status-chip--<?= e($paymentStatus) ?>"><?= e($statusLabels[$paymentStatus] ?? ucfirst($paymentStatus)) ?></span></td><td data-label="Ticket"><strong class="min-w-0 break-all"><?= e($participant['ticket_number'] ?? 'Not issued') ?></strong><small><span class="status-chip status-chip--<?= e($ticketStatus) ?>"><?= e($statusLabels[$ticketStatus] ?? ucfirst($ticketStatus)) ?></span></small></td><td data-label="Attendance"><span class="status-chip status-chip--<?= e($attendanceStatus) ?>"><?= e($statusLabels[$attendanceStatus] ?? ucfirst($attendanceStatus)) ?></span><?php if (!empty($participant['scanned_at'])): ?><small><?= e($participant['scanned_at']) ?></small><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div>
