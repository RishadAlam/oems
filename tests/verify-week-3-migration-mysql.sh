...
expect "$(scalar "SELECT COUNT(*) FROM settings WHERE (\`group\`, \`key\`) IN (('mail', 'outbox_max_attempts'), ('mail', 'reminder_lead_hours'), ('operations', 'backup_retention')) AND is_public = 0")" '3' 'private Week 3 settings'
