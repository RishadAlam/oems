...
if [ "${OEMS_OUTBOX_TEST_MYSQL:-}" != "1" ]; then
    echo 'Set OEMS_OUTBOX_TEST_MYSQL=1 to run the outbox verifier.' >&2
    exit 2
fi

mysql_host="${OEMS_OUTBOX_TEST_HOST:-127.0.0.1}"
mysql_port="${OEMS_OUTBOX_TEST_PORT:-3306}"
mysql_user="${OEMS_OUTBOX_TEST_USER:-root}"
mysql_password="${OEMS_OUTBOX_TEST_PASSWORD:-}"
database="${OEMS_OUTBOX_TEST_DATABASE:-oems_outbox_$$_$(date +%s)}"
database_owned=false

case "$database" in
    oems_outbox_*) ;;
    *) echo 'The verifier only accepts disposable oems_outbox_* database names.' >&2; exit 2 ;;
esac
case "$database" in
...
DB_HOST="$mysql_host" DB_PORT="$mysql_port" DB_DATABASE="$database" DB_USERNAME="$mysql_user" DB_PASSWORD="$mysql_password" php tests/verify-outbox-concurrency-mysql.php
