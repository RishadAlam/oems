...
final class PlatformSettingsRepositoryTest extends TestCase
{
    public function testReadsOnlyRequestedPublicKeysAndTransactionallyUpsertsCatalogValues(): void
    {
        ...
        $pdo->exec('CREATE TABLE activity_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, action TEXT, subject_type TEXT, subject_id INTEGER NULL, description TEXT, properties TEXT)');
        ...
        $repository->setMaintenance(true, 7);
        $this->assertSame(['maintenance_mode' => '1'], $repository->privateValuesForKeys(['maintenance_mode', 'smtp_password']));
        $this->assertSame('maintenance_enabled', $pdo->query('SELECT action FROM activity_logs')->fetchColumn());
        $repository->setMaintenance(true, 7);
