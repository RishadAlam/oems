...
final class RegistrationServiceTest extends TestCase
{
    ...
    private function createSchema(): void
    {
        ...
        $this->connection->exec('CREATE TABLE events (id INTEGER PRIMARY KEY, organizer_id INTEGER NOT NULL, category_id INTEGER NOT NULL, venue_id INTEGER NULL, title TEXT NOT NULL, slug TEXT NOT NULL, start_date TEXT NOT NULL, registration_deadline TEXT NOT NULL, capacity INTEGER NOT NULL, ticket_price NUMERIC NOT NULL, currency TEXT NOT NULL, status TEXT NOT NULL, deleted_at TEXT NULL)');
    ...
    private function seedRows(): void
    {
        ...
        $this->connection->exec("INSERT INTO events (id, organizer_id, category_id, venue_id, title, slug, start_date, registration_deadline, capacity, ticket_price, currency, status, deleted_at) VALUES
            (10, 1, 1, 1, 'Free Event', 'free-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 2, 0, 'BDT', 'published', NULL),
            (11, 1, 1, 1, 'Paid Event', 'paid-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 2, 125.50, 'BDT', 'published', NULL),
            (12, 1, 1, 1, 'Draft Event', 'draft-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 2, 0, 'BDT', 'draft', NULL),
            (13, 1, 2, 1, 'Inactive Category', 'inactive-category', datetime('now', '+10 days'), datetime('now', '+9 days'), 2, 0, 'BDT', 'published', NULL),
            (14, 1, 1, 1, 'Closed Event', 'closed-event', datetime('now', '+10 days'), datetime('now', '-1 minute'), 2, 0, 'BDT', 'published', NULL),
            (15, 1, 1, 1, 'Started Event', 'started-event', datetime('now', '-1 minute'), datetime('now', '-1 day'), 2, 0, 'BDT', 'published', NULL),
            (16, 1, 1, 1, 'Full Event', 'full-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 1, 0, 'BDT', 'published', NULL),
            (17, 1, 1, 1, 'Reject Event', 'reject-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 1, 80, 'BDT', 'published', NULL)");
        ...
        $this->connection->exec("INSERT INTO events (id, organizer_id, category_id, venue_id, title, slug, start_date, registration_deadline, capacity, ticket_price, currency, status, deleted_at) VALUES (18, 1, 1, 1, 'Attendance Event', 'attendance-event', datetime('now', '+10 days'), datetime('now', '+9 days'), 1, 0, 'BDT', 'published', NULL)");
    }
